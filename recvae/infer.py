...
parser.add_argument('--return-latent', default=False, action="store_true")
...
print(np.unique(infer_data_tr.toarray()[:100]))
...
def get_ratings_pred(model, data_in, data_out, samples_perc_per_epoch=1, batch_size=500):
    model.eval()
    result = []
...
def get_latent(model, data_in, data_out, samples_perc_per_epoch=1, batch_size=500):
    model.eval()
    result = []
    ...
    for batch in generate(batch_size=batch_size,
        ...
        latent = model(ratings_in, calculate_loss=False, return_latent=True).cpu().detach().numpy()
...
if args.return_latent:
    result = get_latent(model, infer_data_tr, infer_data_te, 1)
    with open(f"{args.dataset}/result_latent.pkl", "wb") as f:
        pickle.dump(result, f)
else:
    result = get_ratings_pred(model, infer_data_tr, infer_data_te, 1)
